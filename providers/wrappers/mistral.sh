...
# Log
echo "[mistral] Processing task with model: $MODEL" >&2
...
# Call Mistral API
echo "[AGENT-PROGRESS] model-wait: Waiting for Mistral API response..." >&2
START_TIME=$(date +%s)
...
if [ "$HTTP_CODE" != "200" ]; then
    echo "ERROR: Mistral API request failed with HTTP $HTTP_CODE" >&2
    cat "$OUTPUT_FILE" >&2
    exit 1
fi
...
if [ -z "$RESPONSE_CONTENT" ]; then
    echo "ERROR: No valid response from Mistral API" >&2
    cat "$OUTPUT_FILE" >&2
    exit 1
fi
...
echo "[mistral] Task completed successfully" >&2
